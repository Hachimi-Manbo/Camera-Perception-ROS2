...
void YOLOv8::postprocess(std::vector<Object>& objs, float score_thres, float iou_thres, int topk, int num_labels)
{
    ...
    for (int i = 0; i < num_anchors; i++) {
        ...
        int label_temp = max_s_ptr - scores_ptr;
        if (score > score_thres && label_temp <= 7 && label_temp != 4 && label_temp != 6) {
    ...
    for (auto& i : indices) {
        ...
        obj.prob  = scores[i];
        obj.label = labels[i];
        if(obj.label<8&&obj.label!=4&&obj.label!=6){
            objs.push_back(obj);
        }
        cnt += 1;
    }
