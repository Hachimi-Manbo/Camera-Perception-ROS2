...
{
    cv::FileStorage fs_read("obj_det2d/yaml/config.yaml", cv::FileStorage::READ);

	// track_thresh = 0.5;
	// high_thresh = 0.6;
	// match_thresh = 0.8;
	fs_read["track_thresh"] >> track_thresh;
	fs_read["high_thresh"] >> high_thresh;
	fs_read["match_thresh"] >> match_thresh;
    fs_read.release();
...
vector<STrack> BYTETracker::update(const vector<det::Object>& objects)
{
