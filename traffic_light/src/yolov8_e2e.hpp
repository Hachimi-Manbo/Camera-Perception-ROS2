...
/*
This module using end-to-end yolov8 model, and the results return int (0-9)
The DL model is trained by myself based on YOLOv8s
The dataset is collect at SuZhou, having 10 labels: 
"green straight", "red straight", "yellow straight",
 "green left", "red left",  "yellow left", 
 "green right", "red right", "yellow right", 
 "unknown"
*/

class YOLOv8_E2E {
public:
    explicit YOLOv8_E2E(const std::string& engine_file_path);
    ~YOLOv8_E2E();
    ...
    const std::vector<std::vector<unsigned int>> COLORS = {
    {  0, 250, 148 }, {  255, 48, 48 }, { 255, 255, 000 }, { 0, 250, 148 }, { 255, 48, 48 },
    { 255, 255, 000}, {  0, 250, 148 }, { 255, 48, 48 }, { 255, 255, 000 }, {  0 , 0, 255 }};
...
YOLOv8_E2E::YOLOv8_E2E(const std::string& engine_file_path)
{
...
void YOLOv8_E2E::make_pipe(bool warmup)
{
...
void YOLOv8_E2E::letterbox(const cv::Mat& image, cv::Mat& out, cv::Size& size)
{
...
void YOLOv8_E2E::copy_from_Mat(const cv::Mat& image)
{
    cv::Mat  nchw;
...
void YOLOv8_E2E::copy_from_Mat(const cv::Mat& image, cv::Size& size)
{
    cv::Mat nchw;
    this->letterbox(image, nchw, size);
...
void YOLOv8_E2E::postprocess(std::vector<Object>& objs)
{
    objs.clear();
...
void YOLOv8_E2E::draw_objects(const cv::Mat&                                image,
                          cv::Mat&                                      res,
                          const std::vector<Object>&                    objs,
                          const std::vector<std::string>&               CLASS_NAMES,
                          const std::vector<std::vector<unsigned int>>& COLORS)
